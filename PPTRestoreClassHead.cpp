...
struct PPTRestore::Ximpl
{
	...
	map<double, Vec4i> find_cross_points_by_edges(const vector<Vec4i>& lines);	//根据直线提取确定矩形4个顶点
	...
	Mat perspective_transformation(const vector<Point2f>&, Mat&);//透视变换
	Mat image_enhance(Mat&);//图强增强
	Debug* debug;
...
map<double, Vec4i> PPTRestore::Ximpl::find_cross_points_by_edges(const vector<Vec4i>& lines)//通过直线找点
{
